...
| **7830**/**8467** over DoH | the DoH client passes the same literal `0` payload size the DoT client used to, so it announces no EDNS(0) and pads nothing — finding 31 on a second transport. Whether RFC 8467's DoT block lengths carry over to an HTTP transport, and what RFC 8484 asks of the HTTP layer, has to be read before it is asserted | nothing; DoT was done first |

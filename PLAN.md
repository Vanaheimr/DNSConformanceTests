...
Counts as of the full 2026-09-15 Windows run: **1072 tests, 1068 ✅, 0 ❌, 4
...
| 2782 | SRV selection | priority walked in order rather than stopping at the lowest, weight-0 targets reachable but rare, equal weights spread ✅ (finding 52) | ✅ |
| 2782, 4343 | SRV endpoints as values | `DNSSRVEndpoint` declares `IEquatable` and `IComparable` and honoured neither: `Equals` returned false unconditionally — `x.Equals(x)` included — while `CompareTo` returned zero unconditionally, so every endpoint was unequal to every other *and* sorted equal to it. `GetHashCode` was built properly from all seven fields, which is what made it quietly wrong: two identical endpoints shared a bucket and then disagreed about being the same one, so `Contains`, `Distinct` and `HashSet` all lost. Now ordered by priority and then weight — the order RFC 2782 walks — with target and port breaking ties, and the target compared case-insensitively per RFC 4343 with the hash folding the same way | ✅ |
| 3597 §2, 4025 §2, 4701 §3.1, 8914 §2 | The shortest RDATA a record can have | a record of a known type arriving shorter than its own fixed fields is still a record, and reading it as zeros is what keeps the stream where the next one begins (finding 21). IPSECKEY with nought, one and two octets of its three; DHCID at exactly its three-octet header, with the fields present and the digest empty, and below it with the fields reporting themselves absent rather than guessing zeros; an Extended DNS Error of exactly its two-octet info code, whose EXTRA-TEXT is absent and not empty | ✅ |
| 9460 §2.1 | A quoted SvcParam value keeps its spaces | §2.1 separates SvcParams on whitespace and lets a value be quoted, which means a quoted value may contain the whitespace that otherwise separates them. `alpn="h2 h3" port=8080` is two parameters and not four, runs of spaces are one separator, and the rule holds for HTTPS as well as SVCB — they carry a copy of the splitter each, which is how findings 31, 32 and 34 happened |
| 4255 §3.1.3, 6594 §3 | An SSHFP fingerprint is as long as its type says | SHA-1 is twenty octets and SHA-256 is thirty-two, counted in octets rather than hex characters, and a record whose fingerprint is shorter is refused ✅ (finding 54) — it used to be accepted with the difference filled in as zeros, which also made this very check unreachable |
| 4025 §2.5 | An IPSECKEY gateway is what its type says | the gateway type octet decides the field's meaning and the length decides nothing: a name of four octets stays a name, a name of sixteen stays a name, and `2.97.98.0` — whose octets are `02 61 62 00`, a perfectly good encoding of `ab.` — stays an address |
| 1876 §3 | LOC's hemispheres, in either case | the letter both sets the sign and advances the cursor, which are two conditions over the same four spellings; a zone file may use either case, and reading `s` as northern puts the location on the wrong side of the equator without failing anything |
| 1035 §2.3.3 | Mnemonics are read in either case | "no significance should be attached to the case" — for the class, for the type, and for the type a RRSIG says it covers, which a second reader parses. The header tries class, TTL and type against the same token, so a case-sensitive attempt at any of them is a record type it will never reach |
| 1035 §4.1.3 | The largest RDATA that can be written down | RDLENGTH is sixteen bits, so 65535 octets of RDATA are exactly expressible and 65536 are not. Twenty-two record types carry a copy of that guard and not one of them had been reached from either side: twelve are driven to the octet here — TXT and SPF counted in character-strings, where the last text that fits is 65279 octets in 256 chunks rather than 65535; URI, whose target carries no length prefix; RRSIG, SIG, TSIG, TKEY, NSEC, SVCB and HTTPS; the RFC 3597 §5 generic form through the reader; and the constructor that refuses it outright. The other ten can never fire, and §2.3.4 is why: their RDATA is one or two domain names plus fixed octets, and a name stops at 255 | ✅ |
| 1876 §2, §3 | A LOC that stops early, and the smallest size it can state | §3's presentation form is brackets inside brackets — minutes, seconds, hemisphere, altitude and three precisions all optional — so every bracket is a comparison against how many tokens actually arrived, and reading one token past the end turns the record into "not a LOC" rather than into a crash. Three lines that stop at three different brackets. And §2's size octet: zero centimetres is a size and not an absent field, and nine centimetres is base nine with power zero — one reduction further writes 1e1, which is ten | ✅ |
| 9460 §2.1, §2.2, §7.1 | What a SvcParam looks like with nothing to say | `SvcParam = SvcParamKey ["=" SvcParamValue]` puts the `=` inside the optional group, so `no-default-alpn` — whose value §7.1 says MUST be empty — is written as the key alone. On the wire the same parameter is the last four octets of the RDATA, which is the least a parameter loop may still enter. And the escaping: §2.1 writes a value as an RFC 1035 §5.1 character-string, printable being 0x20 through 0x7E with both ends inside — and each end is its own comparison | ✅ |
| 5155 §3.3 | An absent salt, and a hash that fills its digits exactly | "represented as '-' (without quotes) when the Salt Length field has value 0" — an empty hex string in its place moves every field behind it one token to the left. And the next hashed owner name as base32hex, pinned against a value computed outside Hermod; that half closes nothing, and says so, because the encoder's inner loop turns out to give the same string read either way | ✅ |
| 1035 §5.1 | A line that has to read back | the type is the one field §5.1 puts no brackets around, so a header that runs out of tokens before reaching it is missing a type and has to say so rather than read one token further. And two records that write six tokens and wanted seven: TSIG (RFC 8945 §4.2) and TKEY (RFC 2930 §2) each refused the presentation form they had just produced | ✅ |
...
| 3123 §5, §5.1, §5.2 | APL: what the format refuses, and what it must not | fifteen items that are not §5 syntax — no address family, an unparseable one, one past sixteen bits, no prefix, a prefix past an octet, an address that does not parse under the family it claims, and a family §4 gives no AFDPART — each refused with nothing left behind for the caller. And the other direction, which no rejection test can see: §5.1's `0..32` and §5.2's `0..128` asserted from *both* sides, so a /32 and a /128 host route stay legal. Found by a mutation sweep, which reported all nine of `TryParse`'s refusals as changeable to acceptance unnoticed | ✅ |
...
| 1035 §5.1, 3597 §5, 4034 §3.2 | What the zone-file reader refuses | a line short of name, class, type and RDATA; an owner name with a label past §2.3.4's 63 octets; a header with no type in it; a type with nothing behind it; a token that is neither a mnemonic nor `TYPEnnn`. A TTL the 32-bit field cannot hold, in every spelling — a bare number past it, units multiplying past it, a digit run no integer type can hold, a number with no unit and a unit with no number. A generic form whose length is not a decimal integer, is past `RDLENGTH`, or disagrees with the octets given. A signature time that is not a time, or outside the seconds since 1970 that §3.2's field counts. And the refusal *names the part that is missing*, which is the difference between a reader that helps and one that only says no | ✅ |
| 4034 §4.1.2, 4035 §5.4 | A type bit map's three ways of saying no | `TypeBitMapContains` is what authenticated denial of existence turns on, and had no direct test at all. A bit that is set is the only yes; a type past the end of its window's bitmap is absent rather than an error; a type whose window is not in the map is absent; and a map whose declared bitmap length runs past the record asserts nothing — the same shape as finding 4, where a length was believed over the octets actually present | ✅ |
...
└── fixtures/
    ...
    │   └── resign.sh                    ← regenerate via WSL dnssec-signzone (30-day signatures)
    ├── bind/                            ← named.conf for the WSL BIND instance
    └── docker/                          ← compose for Knot/Unbound/CoreDNS (optional)
```

**The signed fixtures are a deadline, not an asset.** `dnssec-signzone` gives
them thirty days, after which every DNSSEC chain verdict turns Bogus at once —
correctly, which is what makes it confusing. Both CI workflows sign before
testing, so neither ever reads the committed set; `FixtureFreshnessTests` is
what catches it on a developer checkout, naming the expiry date and the script
rather than letting twenty tests blame the validator. It also watches the other
bound of RFC 4034 §3.1.5, since a signature made on a clock that ran ahead fails

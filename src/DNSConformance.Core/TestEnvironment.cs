...
public static class TestEnvironment
{
    ...
    public static void RequireWsl(params String[] tools)
    {
        ...
        if (!Wsl.IsAvailable)
            Assert.Ignore("WSL is not available — skipping. Install WSL and: apt-get install bind9-dnsutils knot-dnsutils ldnsutils bind9 bind9utils");

        foreach (var tool in tools)
            if (!Wsl.HasTool(tool))
                Assert.Ignore($"'{tool}' not found inside WSL — skipping. Install it, e.g.: wsl -u root apt-get install -y bind9-dnsutils knot-dnsutils ldnsutils bind9 bind9utils");
